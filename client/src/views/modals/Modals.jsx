...
import { callApproveForAll, callClaim, 
  callStakeNFT, callUnstakeNFT, callSell, callBuy, 
  callCancelSell, callEdit, callApproveForErc20,
  callStakeAllNFT, callUnstakeAllNFT, callClaimAll } from "utils/calls";
...
import { updateClaimAll, updateStakingItem, updateUnstakeAll, updateStakeAll } from "state/staking";
        ...
        <>
          ...
          <div className=" w-full bg-sr  rounded-xl my-3 py-1">
            ...
            <div className="  w-full rounded-xl   py-2 px-4 flex items-center justify-between">
              ...
              <p className=" text-white">{`${Math.floor(data.stakingTime/DAY_SECOND)*REWARD_PER_DAY} $TT`}</p>
            </div>
            {/* <div className="  w-full rounded-xl   py-2 px-4 flex items-center justify-between">
...
export const ClaimAllModal = ({ setClaimAllModal, data, reward }) => {
  const [proceed, setProceed] = useState(false)
  const [pendingTx, setPendingTx] = useState(false)
  const [successTx, setSuccessTx] = useState(false)
  const [hash, setHash] = useState("")
  ...
  const handleClaim = async () => {
    ...
    try {
      if (web3Context?.provider && web3Context?.account) {
        ...
        // await callApproveForAll(web3Context?.provider, web3Context?.account, getStakingAddress(), true)
        let ids = []
        for(var i = 0; i < data.length; i++) ids.push(Number(data[i].id))
        const _hash = await callClaimAll(web3Context?.provider, web3Context?.account, ids)
        dispatch(updateClaimAll({ids: ids}))
        setHash(_hash.transactionHash)
        setPendingTx(false)
        setSuccessTx(true)
        // const body = {
  ...
  return (
    <div>
      <div className=" w-full flex items-center justify-between px-3 py-2 border-gray-800 mb-4 border-b">
        <h1 className=" text-xl text-white">Claim rewards for your item</h1>
        <VscChromeClose
          onClick={() => setClaimAllModal(false)}
          className=" w-5 h-5 cursor-pointer text-white "
        />
      </div>
      {proceed === false ? (
        <>
          ...
          <div className=" w-full bg-sr  rounded-xl my-3 py-1">
            ...
            <div className="  w-full rounded-xl   py-2 px-4 flex items-center justify-between">
              ...
              <p className=" text-white">{`${reward.toFixed(2)} $TT`}</p>
            </div>
            {/* <div className="  w-full rounded-xl   py-2 px-4 flex items-center justify-between">
              <p className=" text-gray-500">Total Fee</p>
              <p className=" text-white">0.03 BNB</p>
            ...
            {pendingTx === true ? (
              ...
              successTx ? (
                <>
                  ...
                  <div className=" pt-8 w-full flex items-center justify-center  gap-5 border-t border-gray-800">
                    <button onClick={()=>setClaimAllModal(false)} className=" py-3 px-10 border-2 w-full  bg-pr border-none  outline-none rounded-3xl text-sr flex items-center justify-center   font-bold">
                      <p className="  font-bold">OK</p>
                    </button>
                  </div>
                </>) : (
...
export const UnStakeModal = ({ setUnstake, data }) => {
  ...
  return (
    <div>
      <div className=" w-full flex items-center justify-between px-3 py-2 border-gray-800 mb-4 border-b">
        <h1 className=" text-xl text-white">Cancel HODL your NFT</h1>
        <VscChromeClose
        ...
        <>
          ...
          <div className=" w-full bg-sr  rounded-xl my-3 py-1">
            <div className=" w-full rounded-xl   py-2 px-4 flex items-center justify-between">
              ...
              <p className=" text-white">{`${data.mintedTokens} $TT`}</p>
            </div>
...
export const UnStakeAllModal = ({ setUnStakeAllModal, data, reward }) => {
  const [proceed, setProceed] = useState(false)
  const [pendingTx, setPendingTx] = useState(false)
  const [successTx, setSuccessTx] = useState(false)
  const [hash, setHash] = useState("")
  ...
  const handleUnStake = async () => {
    ...
    try {
      if (web3Context?.provider && web3Context?.account) {
        ...
        let ids = []
        for(var i = 0; i < data.length; i++) ids.push(Number(data[i].id))
        const _hash = await callUnstakeAllNFT(web3Context?.provider, web3Context?.account, ids)
        // dispatch(updateOwner({id: data.id, owner: web3Context?.account}))
        // dispatch(updateStakingItem({id: data.id, owner: NULL_ADDRESS, startTime: 0, stakedDays: data.stakedDays}))
        // dispatch(fetchNFTDetailDataSync({id: data.id}))
        dispatch(updateUnstakeAll({ids: ids}))
        setHash(_hash.transactionHash)
        setPendingTx(false)
        setSuccessTx(true)
        // const body = {
  ...
  return (
    <div>
      <div className=" w-full flex items-center justify-between px-3 py-2 border-gray-800 mb-4 border-b">
        <h1 className=" text-xl text-white">Cancel HODL your All NFT</h1>
        <VscChromeClose
          onClick={() => setUnStakeAllModal(false)}
          className=" w-5 h-5 cursor-pointer text-white "
        />
      </div>
      {proceed === false ? (
        <>
          ...
          <div className=" w-full bg-sr  rounded-xl my-3 py-1">
            <div className=" w-full rounded-xl   py-2 px-4 flex items-center justify-between">
              <p className=" text-gray-500">Total Rewards</p>
              <p className=" text-white">{`${reward} $TT`}</p>
            </div>
            ...
            {pendingTx === true ? (
              ...
              successTx ? (
                <>
                  ...
                  <div className=" pt-8 w-full flex items-center justify-center  gap-5 border-t border-gray-800">
                    <button onClick={()=>setUnStakeAllModal(false)} className=" py-3 px-10 border-2 w-full  bg-pr border-none  outline-none rounded-3xl text-sr flex items-center justify-center   font-bold">
                      <p className="  font-bold">OK</p>
                    </button>
                  </div>
                </>) : (
...
export const StakeModal = ({ setStake, data }) => {
  ...
  return (
    <div>
      <div className=" w-full flex items-center justify-between px-3 py-2 border-gray-800 mb-4 border-b">
        <h1 className=" text-xl text-white">
          HODL <span className=" text-pr">{data.name}</span>
        </h1>
        <VscChromeClose
        ...
        <>
          ...
          <div className=" pt-8 w-full grid grid-cols-2 gap-5 border-t border-gray-800">
            ...
            <button
              ...
              className=" py-3 w-full border-none outline-none rounded-3xl text-br flex items-center justify-center  bg-pr font-bold"
            >
                <p className=" text-br font-bold">HODL</p>    
            </button>
          </div>
        </>
      ) : (
...
export const StakeAllModal = ({ setStakeAll, data }) => {
  // const [pendingApproval, setPendingApproval] = useState(false)
  const [proceed, setProceed] = useState(false)
  const [pendingTx, setPendingTx] = useState(false)
  const [successTx, setSuccessTx] = useState(false)
  ...
  const handleStake = async () => {
    ...
    try {
      if (web3Context?.provider && web3Context?.account) {
        ...
        let ids = []
        for(var i = 0; i < data.length; i++) ids.push(Number(data[i].id))
        const _hash = await callStakeAllNFT(web3Context?.provider, web3Context?.account, ids)
        // dispatch(updateOwner({id: data.id, owner: getStakingAddress()}))
        // dispatch(updateStakingItem({id: data.id, owner: web3Context?.account, startTime: new Date() + 120, stakedDays: data.stakedDays}))
        // dispatch(fetchNFTDetailDataSync({id: data.id}))
        dispatch(updateStakeAll({ids: ids, account: web3Context?.account}))
        setHash(_hash.transactionHash)
        setPendingTx(false)
        setSuccessTx(true)
        // const body = {
        ...
        <>
          ...
          <div className=" pt-8 w-full grid grid-cols-2 gap-5 border-t border-gray-800">
            ...
            <button
              ...
              className=" py-3 w-full border-none outline-none rounded-3xl text-br flex items-center justify-center  bg-pr font-bold"
            >
                <p className=" text-br font-bold">HODL All</p>    
            </button>
          </div>
        </>
      ) : (
            ...
            {pendingTx === true ? (
              ...
              successTx ? (
                <>
                  ...
                  <div className=" pt-8 w-full flex items-center justify-center  gap-5 border-t border-gray-800">
                    <button onClick={()=>setStakeAll(false)} className=" py-3 px-10 border-2 w-full  bg-pr border-none  outline-none rounded-3xl text-sr flex items-center justify-center   font-bold">
                      <p className="  font-bold">OK</p>
                    </button>
                  </div>
                </>) : (
