...
import { register } from 'utils/calls'

import { fetchRegisterState, fetchSponsorAddressFormAccount } from './fetchSponsorAddress'
...
const Signup = () => {
    ...
    //         try {
    //             await register(web3Context?.provider, web3Context?.account, referralCode)
    //             const body = {
    ...
    const singupWithoutRef = async () => {
        ...
        if (web3Context?.provider && web3Context?.account) {
            try {
                await register(web3Context?.provider, web3Context?.account)
                const body = {
                    public_key:  web3Context?.account.toLowerCase()
                }
        ...
        <div className=' w-full'>
            ...
            <GlobModal size="sm" open={modalOpen2} setOpen={setModalOpen2} >
                <div >
                    ...
                    <p className=' text-sm text-center text-white py-8'>Register into the smart contract.</p>
                    ...
                    <div className=' grid grid-cols-2 gap-2 py-3'>
                        ...
                        <button onClick={() => {
                            setModalOpen2(false)
                            singupWithoutRef()
                        }} className=' w-full bg-pr  py-2 text-sm rounded-3xl text-br border-2 border-pr'>Registration</button>
                    </div>
                </div>
            </GlobModal>
